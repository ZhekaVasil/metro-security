...
export const Question = ({ questionId, setQuestions, questions, className }) => {
  ...
  return (
    <div className={cx(className, classes.container)}>
      <Header as="h3">{Parser(question)}</Header>
      <Form>
        {answers.map(({ id, answer }) => (
          <Form.Field key={id}>
            <Checkbox label={Parser(answer)} name="radioGroup" value={id} checked={answeredIds.includes(id)} onChange={handleCheckboxChange} />
          </Form.Field>
        ))}
      </Form>
    </div>
